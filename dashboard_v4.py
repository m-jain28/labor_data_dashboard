...
with left:
    ...
    if county_name == "(None)":
        ...
        if trend.empty:
            ...
            # === Dynamic overlay: U.S. avg if All states, else the state's avg ===
            ...
            # Dashed comparison average (same blue, lighter)
...
with right:
    # Read the county selected on the left
    ...
    if not county_name or county_name == "(None)":
        ...
        # Build the comparison distribution for the selected year + metric
        comp_df = (
        ...
        if comp_df.empty:
            ...
            # Histogram (rounded bars), dashed scope average, solid county line
            hist = (
                alt.Chart(comp_df)
                .mark_bar(color="#A4A6A8", opacity=0.95,
            ...
            if county_val is not None:
                ...
                layers.append(county_rule)

            st.altair_chart(alt.layer(*layers), use_container_width=True)

            # Pretty ordinal for percentile text
            ...
            if county_val is not None:
                ...
                st.caption(
                    f"{county_name} is at the **{ordinal(pct)} percentile** within the **{scope_name}** in **{year}**."
                )
            else:
